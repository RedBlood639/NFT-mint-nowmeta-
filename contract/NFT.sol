...
contract RaffleNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("Raffle NFT", "RNFT") {
        admin = msg.sender;
    }
    ...
    function customMint() public {        
        ...
        _mint(msg.sender, newItemId);
        string memory idstr;
        if(newItemId < 10 || newItemId == 0) {
            idstr = concatenate("000", uintToString(newItemId));
        }
        else if (newItemId >= 10 && newItemId < 100) {
            idstr = concatenate("00", uintToString(newItemId));
        }
        else if (newItemId >= 100 && newItemId < 1000) {
            idstr = concatenate("0", uintToString(newItemId));
        }
        else {
            idstr = uintToString(newItemId);
        }

        string memory extension = concatenate(idstr, ".jpg");

        string memory path = concatenate("ipfs://QmVpFMAK3DzLaokd8xsG8aPSYS86M8eGnDKC82Rdekighw/", extension);


        super._setTokenURI(newItemId, path);
